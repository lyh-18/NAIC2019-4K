...
class DynamicUpsamplingFilter_3C(nn.Module):
    ...
    def __init__(self, filter_size=(1, 5, 5)):
        ...
        #print('expand_filter_np: ', expand_filter_np.shape)  # (25, 1, 5, 5)
...
class DUF_16L(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv3d_1(x)    # in: [B, C, T, H, W]   out: [B, 64, 7, H, W]
        #print('x1 size: ',x.size())
        x = self.dense_block_1(x) # in: [B, 64, 7, H, W]   out:
...
class DFN_16L(nn.Module):
    '''Official DUF structure with 16 layers'''

    def __init__(self, scale=1, adapt_official=False):
        super(DFN_16L, self).__init__()
        self.conv3d_1 = nn.Conv3d(3, 64, (1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1), bias=True)
        self.dense_block_1 = DenseBlock(64, 64 // 2, t_reduce=False)  # 64 + 32 * 3 = 160, T = 7
        self.dense_block_2 = DenseBlock(160, 64 // 2, t_reduce=True)  # 160 + 32 * 3 = 256, T = 1
        self.bn3d_2 = nn.BatchNorm3d(256, eps=1e-3, momentum=1e-3)
...
class DenseBlock_2d(nn.Module):
    ...
    def __init__(self, nf=64, ng=32, t_reduce=False):
        ...
        self.bn2d_1 = nn.BatchNorm2d(nf, eps=1e-3, momentum=1e-3)
        self.conv2d_1 = nn.Conv2d(nf, nf, (1, 1), stride=(1, 1), padding=(0, 0), bias=True)
        self.bn2d_2 = nn.BatchNorm2d(nf, eps=1e-3, momentum=1e-3)
        self.conv2d_2 = nn.Conv2d(nf, ng, (3, 3), stride=(1, 1), padding=pad, bias=True)
        self.bn2d_3 = nn.BatchNorm2d(nf + ng, eps=1e-3, momentum=1e-3)
        self.conv2d_3 = nn.Conv2d(nf + ng, nf + ng, (1, 1), stride=(1, 1), padding=(0, 0),
                                  bias=True)
        self.bn2d_4 = nn.BatchNorm2d(nf + ng, eps=1e-3, momentum=1e-3)
        self.conv2d_4 = nn.Conv2d(nf + ng, ng, (3, 3), stride=(1, 1), padding=pad, bias=True)
        self.bn2d_5 = nn.BatchNorm2d(nf + 2 * ng, eps=1e-3, momentum=1e-3)
        self.conv2d_5 = nn.Conv2d(nf + 2 * ng, nf + 2 * ng, (1, 1), stride=(1, 1),
                                  padding=(0, 0), bias=True)
        self.bn2d_6 = nn.BatchNorm2d(nf + 2 * ng, eps=1e-3, momentum=1e-3)
        self.conv2d_6 = nn.Conv2d(nf + 2 * ng, ng, (3, 3), stride=(1, 1), padding=pad,
                                  bias=True)
    ...
    def forward(self, x):
        ...
        x1 = self.conv2d_1(F.relu(self.bn2d_1(x), inplace=True))
        x1 = self.conv2d_2(F.relu(self.bn2d_2(x1), inplace=True))
        
        x1 = torch.cat((x, x1), 1)

        x2 = self.conv2d_3(F.relu(self.bn2d_3(x1), inplace=True))
        x2 = self.conv2d_4(F.relu(self.bn2d_4(x2), inplace=True))
        
        x2 = torch.cat((x1, x2), 1)

        x3 = self.conv2d_5(F.relu(self.bn2d_5(x2), inplace=True))
        x3 = self.conv2d_6(F.relu(self.bn2d_6(x3), inplace=True))
        
        x3 = torch.cat((x2, x3), 1)
        return x3

class DFN_16L_2d(nn.Module):
    '''Official DUF structure with 16 layers'''

    def __init__(self, scale=1, adapt_official=False, res=True):
        super(DFN_16L_2d, self).__init__()
        self.res = res
        self.conv2d_1 = nn.Conv2d(3, 64, (3, 3), stride=(1, 1), padding=(1, 1), bias=True)
        self.dense_block_1 = DenseBlock_2d(64, 64 // 2, t_reduce=False)  # 64 + 32 * 3 = 160, T = 7
        self.dense_block_2 = DenseBlock_2d(160, 64 // 2, t_reduce=True)  # 160 + 32 * 3 = 256, T = 1
        self.bn2d_2 = nn.BatchNorm2d(256, eps=1e-3, momentum=1e-3)
        self.conv2d_2 = nn.Conv2d(256, 256, (3, 3), stride=(1, 1), padding=(1, 1),
                                  bias=True)

        self.conv2d_r1 = nn.Conv2d(256, 256, (3, 3), stride=(1, 1), padding=(1, 1),
                                   bias=True)
        self.conv2d_r2 = nn.Conv2d(256, 3 * (scale**2), (3, 3), stride=(1, 1),
                                   padding=(1, 1), bias=True)
        self.conv2d_w1 = nn.Conv2d(256, 256, (3, 3), stride=(1, 1), padding=(1, 1),
                                   bias=True)
        self.conv2d_w2 = nn.Conv2d(256, 64, (3, 3), stride=(1, 1),
                                   padding=(1, 1), bias=True)
        self.conv2d_w3 = nn.Conv2d(64, 3, (3, 3), stride=(1, 1),
                                   padding=(1, 1), bias=True)

        self.conv2d_f1 = nn.Conv2d(256, 512, (1, 1), stride=(1, 1), padding=(0, 0),
                                   bias=True)
        self.conv2d_f2 = nn.Conv2d(512, 1 * 5 * 5 * (scale**2), (1, 1), stride=(1, 1),
                                   padding=(0, 0), bias=True)
    ...
    def forward(self, x):
        ...
        x = self.conv2d_1(x)    # in: [B, C, T, H, W]   out: [B, 64, 7, H, W]
        #print('x1 size: ',x.size())
        x = self.dense_block_1(x) # in: [B, 64, 7, H, W]   out:
        #print('x2 size: ',x.size())
        x = self.dense_block_2(x)  # reduce T to 1
        #print('x3 size: ',x.size())
        x = F.relu(self.conv2d_2(F.relu(self.bn2d_2(x), inplace=True)), inplace=True)
        #print('x4 size: ',x.size())

        # image residual
        if self.res:
            Rx = self.conv2d_r2(F.relu(self.conv2d_r1(x), inplace=True))  # [B, 3*16, 1, H, W]
        
        # weight
        Wx = self.conv2d_w1(x)
        Wx = F.relu(Wx, inplace=True)
        Wx = self.conv2d_w2(Wx)
        Wx = F.relu(Wx, inplace=True)
        Wx = self.conv2d_w3(Wx)

        # filter
        Fx = self.conv2d_f2(F.relu(self.conv2d_f1(x), inplace=True))  # [B, 25*16, 1, H, W]
        Fx = F.softmax(Fx.view(B, 25, self.scale**2, H, W), dim=1)
...
class DFN_16L_2d_1x1(nn.Module):
    '''Official DUF structure with 16 layers'''

    def __init__(self, scale=1, adapt_official=False):
        super(DFN_16L_2d_1x1, self).__init__()
        ...
        self.conv2d_r1 = nn.Conv2d(256, 256, (1, 1), stride=(1, 1), padding=(0, 0),
                                   bias=True)
        self.conv2d_r2 = nn.Conv2d(256, 3 * (scale**2), (1, 1), stride=(1, 1),
                                   padding=(0, 0), bias=True)
        self.conv2d_w1 = nn.Conv2d(256, 256, (1, 1), stride=(1, 1), padding=(0, 0),
                                   bias=True)
        self.conv2d_w2 = nn.Conv2d(256, 3, (1, 1), stride=(1, 1),
                                   padding=(0, 0), bias=True)
    ...
    def forward(self, x):
        ...
        # weight
        Wx = self.conv2d_w2(F.relu(self.conv2d_w1(x), inplace=True))
...
class DFN_16L_2d_alpha(nn.Module):
    '''Official DUF structure with 16 layers'''

    def __init__(self, scale=1, adapt_official=False):
        super(DFN_16L_2d_alpha, self).__init__()
        self.conv2d_1 = nn.Conv2d(3, 64, (3, 3), stride=(1, 1), padding=(1, 1), bias=True)
        self.dense_block_1 = DenseBlock_2d(64, 64 // 2, t_reduce=False)  # 64 + 32 * 3 = 160, T = 7
        self.dense_block_2 = DenseBlock_2d(160, 64 // 2, t_reduce=True)  # 160 + 32 * 3 = 256, T = 1
        self.bn2d_2 = nn.BatchNorm2d(256, eps=1e-3, momentum=1e-3)
        ...
        self.conv2d_r1 = nn.Conv2d(256, 256, (3, 3), stride=(1, 1), padding=(1, 1),
                                   bias=True)
        self.conv2d_r2 = nn.Conv2d(256, 64, (3, 3), stride=(1, 1),
                                   padding=(1, 1), bias=True)
        self.conv2d_r3 = nn.Conv2d(64, 3 * (scale**2), (3, 3), stride=(1, 1),
                                   padding=(1, 1), bias=True)
        self.pooling2d_r4 = nn.AdaptiveAvgPool2d(1)
        
        
        self.conv2d_w1 = nn.Conv2d(256, 256, (1, 1), stride=(1, 1), padding=(0, 0),
                                   bias=True)
        self.conv2d_w2 = nn.Conv2d(256, 64, (1, 1), stride=(1, 1),
                                   padding=(0, 0), bias=True)
        self.conv2d_w3 = nn.Conv2d(64, 3 * (scale**2), (1, 1), stride=(1, 1),
                                   padding=(0, 0), bias=True)
        self.pooling2d_w4 = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        ...
        Rx = self.conv2d_r2(Rx)
        Rx = F.relu(Rx, inplace=True)
        Rx = self.conv2d_r3(Rx)
        Rx = self.pooling2d_r4(Rx)
        ...
        Wx = F.relu(Wx, inplace=True)
        Wx = self.conv2d_w3(Wx)
        Wx = self.pooling2d_w4(Wx)
        
        #print('Wx size', Wx.size())    # torch.Size([1, 3, 1, 1])
        ...
        #print('1111', out.size())
        #print('2222', Rx.size())
        #print('out size:',out.size())   # torch.Size([1, 3, 256, 256])
...
class DCCN_16L_2d(nn.Module):
    '''Official DUF structure with 16 layers'''

    def __init__(self, scale=1, adapt_official=False):
        super(DCCN_16L_2d, self).__init__()
        self.conv2d_1 = nn.Conv2d(3, 64, (3, 3), stride=(1, 1), padding=(1, 1), bias=True)
        self.dense_block_1 = DenseBlock_2d(64, 64 // 2, t_reduce=False)  # 64 + 32 * 3 = 160, T = 7
        self.dense_block_2 = DenseBlock_2d(160, 64 // 2, t_reduce=True)  # 160 + 32 * 3 = 256, T = 1
        self.bn2d_2 = nn.BatchNorm2d(256, eps=1e-3, momentum=1e-3)
        ...
        self.conv2d_w1 = nn.Conv2d(256, 256, (1, 1), stride=(1, 1), padding=(0, 0),
                                   bias=True)
        self.conv2d_w2 = nn.Conv2d(256, 3 * (scale**2), (1, 1), stride=(1, 1),
                                   padding=(0, 0), bias=True)
    ...
    def forward(self, x):
        ...
        # weight
        Wx = self.conv2d_w2(F.relu(self.conv2d_w1(x), inplace=True))  # [B, 3*16, 1, H, W]
...
class DCCN_16L_2d_alpha(nn.Module):
    '''Official DUF structure with 16 layers'''

    def __init__(self, scale=1, adapt_official=False):
        super(DCCN_16L_2d_alpha, self).__init__()
        self.conv2d_1 = nn.Conv2d(3, 64, (3, 3), stride=(1, 1), padding=(1, 1), bias=True)
        self.dense_block_1 = DenseBlock_2d(64, 64 // 2, t_reduce=False)  # 64 + 32 * 3 = 160, T = 7
        self.dense_block_2 = DenseBlock_2d(160, 64 // 2, t_reduce=True)  # 160 + 32 * 3 = 256, T = 1
        self.bn2d_2 = nn.BatchNorm2d(256, eps=1e-3, momentum=1e-3)
        ...
        self.conv2d_w2 = nn.Conv2d(256, 64, (3, 3), stride=(1, 1),
                                   padding=(1, 1), bias=True)
        self.conv2d_w3 = nn.Conv2d(64, 3 * (scale**2), (3, 3), stride=(1, 1),
                                   padding=(1, 1), bias=True)
        self.pooling2d_w4 = nn.AdaptiveAvgPool2d(1)
