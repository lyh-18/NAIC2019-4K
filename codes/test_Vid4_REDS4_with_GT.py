...
def main():
    ...
    if data_mode == 'Vid4':
        N_in = 5 #7  # use N_in images to restore one HR image
    else:
        N_in = 5
    ...
    for subfolder, subfolder_GT in zip(subfolder_l, subfolder_GT_l):
        ...
        for img_idx, img_path in enumerate(img_path_l):
            ...
            logger.info('{:3d} - {:25} \tPSNR: {:.4f} dB'.format(img_idx + 1, img_name, crt_psnr))
        ...
        logger.info('Folder {} - Average PSNR: {:.4f} dB for {} frames; '
                    'Center PSNR: {:.4f} dB for {} frames; '
                    'Border PSNR: {:.4f} dB for {} frames.'.format(subfolder_name, avg_psnr,
                                                                   (N_center + N_border),
    ...
    for subfolder_name, psnr, psnr_center, psnr_border in zip(subfolder_name_l, avg_psnr_l,
        ...
        logger.info('Folder {} - Average PSNR: {:.4f} dB. '
                    'Center PSNR: {:.4f} dB. '
                    'Border PSNR: {:.4f} dB.'.format(subfolder_name, psnr, psnr_center,
                                                     psnr_border))
    ...
    logger.info('Save images: {}'.format(save_imgs))
    logger.info('Flip test: {}'.format(flip_test))
    logger.info('Total Average PSNR: {:.4f} dB for {} clips. '
                'Center PSNR: {:.4f} dB. Border PSNR: {:.4f} dB.'.format(
