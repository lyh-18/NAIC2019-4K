...
class PCD_Align(nn.Module):
    ...
    def forward(self, nbr_fea_l, ref_fea_l):
        ...
        # cropping !!!
        if L2_offset.size() != L3_offset.size():
            min_h = min(L2_offset.size()[2],L3_offset.size()[2])
            min_w = min(L2_offset.size()[3],L3_offset.size()[3])
            L2_offset = L2_offset[:, :, 0:min_h, 0:min_w]
            L3_offset = L3_offset[:, :, 0:min_h, 0:min_w]
        ...
        # cropping !!!
        if L2_fea.size() != L3_fea.size():
            min_h = min(L2_fea.size()[2],L3_fea.size()[2])
            min_w = min(L2_fea.size()[3],L3_fea.size()[3])
            L2_fea = L2_fea[:, :, 0:min_h, 0:min_w]
            L3_fea = L3_fea[:, :, 0:min_h, 0:min_w]
...
class TSA_Fusion(nn.Module):
    ...
    def forward(self, aligned_fea):
        ...
        # cropping !!!
        if att.size() != att_L.size():
            min_h = min(att.size()[2],att_L.size()[2])
            min_w = min(att.size()[3],att_L.size()[3])
            att = att[:, :, 0:min_h, 0:min_w]
            att_L = att_L[:, :, 0:min_h, 0:min_w]
...
class FusionDenoise(nn.Module):
    def __init__(self, nf=64):
        super(FusionDenoise, self).__init__()
...
class MYEDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        self.denoise_extract_fea = nn.Sequential(nn.ReplicationPad3d(1),
                                       nn.Conv3d(nf, nf, kernel_size=(3,3,3), stride=1, padding=(0,0,0), bias=True))
    ...
    def forward(self, x):
        ...
        # denoising branch
        aligned_fea_denoise = aligned_fea.permute(0, 2, 1, 3, 4)   # 3D: [B, C, D, H, W]
        aligned_fea_denoise = self.denoise_extract_fea(aligned_fea_denoise)
        denoise_out = self.denoise_block(aligned_fea_denoise)
        ...
        out += base
        return out, denoise_out

class MYEDVR_RES(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        ### HF enhancer
        self.HF_enhancer = nn.Sequential(nn.Conv2d(3, nf, 3, 1, 1, bias=True),
