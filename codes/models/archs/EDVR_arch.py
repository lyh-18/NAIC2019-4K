...
class EDVR(nn.Module):
    ...
    def forward(self, x):
        ...
        aligned_fea = torch.stack(aligned_fea, dim=1)  # [B, N, C, H, W]   torch.Size([16, 5, 64, 64, 64])
        

        if not self.w_TSA:
            aligned_fea = aligned_fea.view(B, -1, H, W)
        fea = self.tsa_fusion(aligned_fea)  # torch.Size([16, 64, 64, 64])

        out = self.recon_trunk(fea) # torch.Size([16, 64, 64, 64])
        out = self.lrelu(self.pixel_shuffle(self.upconv1(out)))  # torch.Size([16, 64, 128, 128])
        out = self.lrelu(self.pixel_shuffle(self.upconv2(out)))  # torch.Size([16, 64, 256, 256])
        out = self.lrelu(self.HRconv(out))  # torch.Size([16, 64, 256, 256])

        out = self.conv_last(out)  # torch.Size([16, 3, 256, 256])
