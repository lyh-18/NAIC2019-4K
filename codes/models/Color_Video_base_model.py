...
class VideoBaseModel(BaseModel):
    def __init__(self, opt):
        ...
        if opt['dist']:
            ...
            self.netC = DistributedDataParallel(self.netC, find_unused_parameters=True, device_ids=[torch.cuda.current_device()])
        else:
    ...
    def test(self):
        self.netG.eval()
        with torch.no_grad():
            self.var_L_corrected = self.netC(self.var_L)
            self.fake_H = self.netG(self.var_L_corrected)
        self.netG.train()
